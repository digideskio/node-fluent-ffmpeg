...
exports = module.exports = function Processor(command) {
  ...
  this._getProgressFromStdErr = function(stderrString, totalDurationSec) {
    ...
    var progress;
    if (ll) {
      progress = ll.split(/frame=([0-9\s]+)fps=([0-9\.\s]+)q=([0-9\.\s]+)(L?)size=([0-9\s]+)kB time=(([0-9]{2}):([0-9]{2}):([0-9]{2}).([0-9]{2})|(\d+\.\d+)) bitrate=([0-9\.\s]+)kbits/ig);    
    }
    if (progress && progress.length > 10) {
      ...
      if (totalDurationSec && totalDurationSec > 0) {
        seconds = /:/.test(ret.timemark) ? this.ffmpegTimemarkToSeconds(ret.timemark) : parseInt(ret.timemark)
        ret.percent = (seconds / totalDurationSec) * 100;
      }
